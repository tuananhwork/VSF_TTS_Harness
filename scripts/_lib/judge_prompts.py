...
- `repeat_rate` / `pivot_rate` / `behavior_class_hint`: tín hiệu CẤU TRÚC —
  `repeat_rate` = tỉ lệ chạy lại cùng tool (kém hiệu quả); `pivot_rate` = tỉ lệ
  user phải bẻ hướng tool của Claude. Cao = session nhiều ma sát.
...
2. Gắn `skill_type`:
   ...
   - `improvement_lesson`: nhóm nhiều ma sát (repeat_rate / pivot_rate cao hoặc
...
chỗ user `pivot` bẻ hướng / Claude `repeat` chạy lại tool).
...
Càng nhiều pivot/repeat vì prompt thiếu ngữ cảnh hay Input/Output không chuẩn
